import LeanBlocksGame.Tactic.Branch
import LeanBlocksGame.Tactic.Hint
...
partial def collectUsedInventory (stx : Syntax) (acc : UsedInventory := {}) : CommandElabM UsedInventory := do
  match stx with
  | .missing => return acc
  | .node _info ``LeanBlocksGame.Tactic.Branch _args => return acc
  | .node _info ``LeanBlocksGame.Tactic.Hint _args => return acc
  ...
  | .atom _info val =>
    ...
    let allowed := LeanBlocksGame.ALLOWED_KEYWORDS
